...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  // Obter transações do mês específico
  ...
  const content = `Gere um relatório com insights sobre minhas finanças, com dicas e orientações de como melhorar minha saúde financeira. As transações estão divididas por ponto e vírgula. A estrutura de cada em é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas: ${transactions.map((transaction) => `${transaction.date.toLocaleDateString("pt-BR")}-R$${transaction.amount}-${transaction.type}-${transaction.category}`).join(";")} `;
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em gestão e organização de finanças pessoais. Você ajuda as pessoas a ornanizarem melhor as suas finanças.",
      },
      {
        role: "user",
        content,
  ...
  // Pegar o relatório gerado pele chatGPT e retornar para o usuário
